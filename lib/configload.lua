...
local global_default_config = {
   ...
   need_build_path_permission = false,
}
...
local function check(config)
   ...
   if config.need_build_path_permission and build_path_permissions then
      ...
      --   error("Current user does not have RW permission for build path '" .. config.base_build_directory .. "'")
         logger:alert("User does NOT have R/W permissions for '" .. config.base_build_directory .. "'!")
      else
         logger:message("User has R/W permissions for '" .. config.base_build_directory .. "'.")
      end
   end
end
